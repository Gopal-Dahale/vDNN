...
float Solver::step(int start_X, int start_y, std::vector<float> &fwd_vdnn_lag,
  ...
  // *)model->params[i]; 			int kernel_size =
  // cur_params->C_in
  // * cur_params->C_out
  // * cur_params->filter_h * cur_params->filter_w; 			if
  // (model->data_type == CUDNN_DATA_FLOAT) {
  // checkCUBLAS(cublasSaxpy(model->cublas_handle, kernel_size,
  // &Salpha,
  // (float
  // *)cur_params->dW, 1, (float *)cur_params->W, 1));

  // 				checkCUBLAS(cublasSaxpy(model->cublas_handle,
  // cur_params->C_out,
  // &Salpha, (float *)cur_params->db, 1, (float
  // *)cur_params->b, 1));
  // 			}
  // 			else if (model->data_type == CUDNN_DATA_DOUBLE) {
  // 				checkCUBLAS(cublasDaxpy(model->cublas_handle,
  // kernel_size,
  // &Dalpha, (double *)cur_params->dW, 1,
  // (double
  // *)cur_params->W, 1));

  // 				checkCUBLAS(cublasDaxpy(model->cublas_handle,
  // cur_params->C_out,
  // &Dalpha, (double *)cur_params->db, 1, (double
  // *)cur_params->b, 1));
  // 			}
  ...
  // 			FCLayerParams *cur_params = (FCLayerParams
  // *)model->params[i]; 			if (model->data_type ==
  // CUDNN_DATA_FLOAT)
  // { 				checkCUBLAS(cublasSaxpy(model->cublas_handle,
  // cur_params->C_in
  // * cur_params->C_out,
  // &Salpha, (float *)cur_params->dW, 1, (float
  // *)cur_params->W, 1));
  ...
  // cur_params->C_in
  // * cur_params->C_out,
  // &Dalpha, (double *)cur_params->dW, 1, (double
  // *)cur_params->W, 1));
  ...
  // (float *)cur_params->dscale, 1, (float
  // *)cur_params->scale, 1));
  // checkCUBLAS(cublasSaxpy(model->cublas_handle, cur_params->sbmv_size,
  // &Salpha,
  // (float
  // *)cur_params->dbias, 1, (float
  // *)cur_params->bias, 1));

  // 			}
  // 			else if (model->data_type == CUDNN_DATA_DOUBLE) {
  // 				checkCUBLAS(cublasDaxpy(model->cublas_handle,
  // cur_params->sbmv_size,
  // &Dalpha, (double *)cur_params->dscale, 1, (double
  // *)cur_params->scale, 1));
  // checkCUBLAS(cublasDaxpy(model->cublas_handle, cur_params->sbmv_size,
  // &Dalpha,
  // (double
  // *)cur_params->dbias, 1, (double
  // *)cur_params->bias, 1));
...
void Solver::train(std::vector<float> &loss, std::vector<int> &val_acc) {
  ...
  std::string file_name =
      "./res/1fb_" + net_name + "_" + std::to_string(bs) + ".txt";
...
void Solver::getComputationTime(
    long num_epoch, std::vector<std::vector<float> > &fwd_computation_time,
  ...
  for (int i = 0; i < num_epoch; i++) {
    for (int j = 0; j < num_train_batches; j++) {
      ...
      stepComputationTime(start_sample, j * batch_size,
                          cur_fwd_computation_time, cur_bwd_computation_time);
...
void Solver::getTransferTime(
    long num_epoch, std::vector<std::vector<float> > &fwd_transfer_time,
...
void Solver::stepComputationTime(int start_X, int start_y,
                                 std::vector<float> &fwd_computation_time,
                                 std::vector<float> &bwd_computation_time) {
  if (model->data_type == CUDNN_DATA_FLOAT)
    model->getComputationTime(&(((float *)X_train)[start_X]), &y_train[start_y],
                              learning_rate, fwd_computation_time,
...
void Solver::stepTransferTime(int start_X, int start_y,
  ...
  if (model->data_type == CUDNN_DATA_FLOAT)
    ...
    model->getTransferTime(&(((double *)X_train)[start_X]), &y_train[start_y],
                           learning_rate, fwd_transfer_time, bwd_transfer_time);
}
