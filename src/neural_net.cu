...
NeuralNet::NeuralNet(std::vector<LayerSpecifier> &layers, DataType data_type,
  ...
  for (int i = 0; i < num_layers; i++) {
    ...
    if (layers[i].type == CONV) {
      ...
      // data_type_size)); checkCudaErrors(cudaMalloc(&dlayer_input[i + 1],
    ...
    // data_type_size)); checkCudaErrors(cudaMalloc(&dlayer_input[i], input_size
    // * data_type_size));
  ...
  // 	if (layers[i].type == CONV) {
  // 		((ConvLayerParams
  // *)params[i])->getWorkspaceSize(cur_workspace_size, free_bytes); if
  // (cur_workspace_size > workspace_size) 			workspace_size =
  // cur_workspace_size;
  // 	}
  // }
...
bool NeuralNet::simulateNeuralNetworkMemory(vDNNConvAlgoPref algo_pref,
  ...
  for (int i = num_layers - 1; i >= 0; i--) {
    ...
    if (layer_type[i] == CONV) {
      ...
      checkWORKSPACE(cur_params->getWorkspaceSize(
          space_tracker.free_bytes - res_bytes, ConvLayerParams::BWD_FILTER,
          algo_pref, hard, cur_filter_workspace_size));
      size_t cur_data_workspace_size = 0;
      if (i > 0)
        checkWORKSPACE(cur_params->getWorkspaceSize(
            space_tracker.free_bytes - res_bytes, ConvLayerParams::BWD_DATA,
            algo_pref, hard, cur_data_workspace_size));
...
void NeuralNet::setOffload(NeuralNet::OffloadType offload_type) {
  if (offload_type == OFFLOAD_NONE) {
    ...
    for (int i = 0; i < num_layers; i++) {
      ...
      if (layer_type[i] == ACTV or layer_type[i] == SOFTMAX or
          layer_type[i] == FULLY_CONNECTED)
        to_offload[i] = false;
      else
        to_offload[i] = true;
    }
    ...
    for (int i = 0; i < num_layers; i++) {
      if (layer_type[i] == CONV) {
        if (toggle == false) to_offload[i] = false;
        toggle = !toggle;
      }
    }
  }
